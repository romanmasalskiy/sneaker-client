...
const MiniCard = (props) => {
  ...
  useEffect(() => {
    if (fetchSneaker) {
      fetch("/id/" + sneaker.styleID + '/prices', {
          headers: myHeaders,
        })
  ...
  const CardText = () => {
    if (minPrice) {
      return (
         <Card.Text className='mini-card-text'>
           <div>From</div>
           <div className='mini-card-price'>${minPrice} <span className='on-text'> on</span><img className='mini-logo'
               src={logo}></img></div>
         </Card.Text>
    );
  }
    ...
    return(
      <a onClick={showCard} style={{ cursor: 'pointer' }} className='card-button'>
        <Card className='mini-card' border="light" tag="a" style={{ cursor: "pointer" }}
